...
if (isset($_POST['beritaKeluar'])) {
  ...
  $query = mysqli_query($conn, "INSERT INTO tbl_berita_acara set nomor = '$nomor', nama_file = '$file_name', pihak_pertama = '$pihak_pertama', pihak_kedua = '$pihak_kedua', ukuran = '$ukuranFile', ekstensi = '$ekstensiFile', berkas = '$linkBerkas'");
  ...
  if ($terupload && $query) {
    ...
    echo 'header(\'location:berita-keluar.php\')';
  } else {
    echo '
      <script>
...
if (isset($_POST['hapusBeritaKeluar'])) {
  ...
  $ambilData = mysqli_query($conn, "SELECT * FROM tbl_berita_acara WHERE id_berita = '$id_berita'");
  ...
  $query = mysqli_query($conn, "DELETE FROM tbl_berita_acara WHERE id_berita = '$id_berita'");
  if ($query) {
    echo '
      <script>
        alert(\'Data Berhasil Dihapus\')
